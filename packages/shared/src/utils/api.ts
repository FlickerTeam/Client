...
const getStorageItem = (key: string): string => {
  const maybeStorage = (globalThis as { localStorage?: { getItem: (k: string) => string | null } })
    .localStorage;
...
export const request = <T = unknown>(
  ...
  const baseUrl = getStorageItem('selectedInstanceUrl');
  const version = getStorageItem('defaultApiVersion');
  const token = getStorageItem('selectedAuthorization');
