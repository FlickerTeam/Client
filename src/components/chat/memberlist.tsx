...
const MemberListItem = ({ member, isTyping, roles }: {
    member: any,
    isTyping: boolean,
    roles: any[]
}): JSX.Element => {
    ...
    const showProfilePopout = (e: React.MouseEvent) => {
        ...
        const bannerUrl = member.user.banner ? `url('${localStorage.getItem("selectedAssetsUrl")!}/avatars/${member.user.id}/${member.user.banner}.png')` : 'none';

        openContextMenu(x, y, (
            <div className="profile-popout-container" onClick={(e) => e.stopPropagation()}>
                {member.user.banner && (
                    <div
                    ...
                    <div className="popout-header">
                        ...
                        {member.user.pronouns && (
                            <div className="pronouns-row">
                                <span className="pronouns" title={`${member.user.username}'s pronouns`}>{member.user.pronouns}</span>
                            </div>
                        )}
                    </div>
                    {member.user.bio && (
                    ...
                    <div className="popout-section">
                        ...
                        <div className="roles-container">
                            {member.roles.map((roleId: any) => {
                                ...
                                return (
                                    <>
                                        <div className="role-pill role-emphasis-border" style={{
                                            borderColor: color
                                        }}>
    ...
    return (
        <div className="member-list-item-wrapper" onContextMenu={(e) => handleRightClick(e)} onClick={(e) => showProfilePopout(e)}>
            ...
            <div className="scroller" onScroll={handleScroll}>
                {items.map((item: any, index: number) => {
                    ...
                    if (item.group && item.group.count > 0) {
                        let role = selectedGuild.roles.find((x: any) => x.id === item.group.id);
                        let group_name = role?.name ?? item.group.id;
